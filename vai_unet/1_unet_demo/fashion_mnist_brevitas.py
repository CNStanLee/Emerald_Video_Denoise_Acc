...
class Denoise(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            bnn.QuantConv2d(1, 16, kernel_size=3, stride=2, padding=1,weight_bit_width=int(w_b)),
            bnn.QuantReLU(bit_width=int(a_b)),
            bnn.QuantConv2d(16, 8, kernel_size=3, stride=2, padding=1,weight_bit_width=int(w_b)),
            bnn.QuantReLU(bit_width=int(a_b))
        )
        
        # Decoder
        self.decoder = nn.Sequential(
            bnn.QuantConvTranspose2d(8, 8, kernel_size=3, stride=2, padding=1, output_padding=1,weight_bit_width=int(w_b)),
            bnn.QuantReLU(bit_width=int(a_b)),
            bnn.QuantConvTranspose2d(8, 16, kernel_size=3, stride=2, padding=1, output_padding=1,weight_bit_width=int(w_b)),
            bnn.QuantReLU(bit_width=int(a_b)),
            bnn.QuantConv2d(16, 1, kernel_size=3, padding=1,weight_bit_width=int(w_b)),
            bnn.QuantSigmoid()
        )
